...
#Data-Preprocessing: converting actors data into numericals
...
library(lattice)
library(car)
histogram(~Gross_Earning_in_Mil | x, data=movies_df)
scatterplot(x =movies_df$Gross_Earning_in_Mil, y=movies_df$Votes) 

library(ggplot2)
bwplot(~, data = movies_df)   
